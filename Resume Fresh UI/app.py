...
def filecallingvoice1():
    ...
    wf.close()           
@app.route('/startaudioonhtml')
def startaudioonhtml():
    print("in start audio html")
    filecallingvoice1()
    return "ok"
...
def audiovideoemotion():
    if 'user' in session:
        if request.method == 'POST':
            ...
            #filecallingvoice()
            countallemotion={}
            t1 = threading.Thread(target=filecallingvoice, args=(10,))
...
def givetest():
    if 'user' in session:
        ...
        # with increment = interval 
            ...
            return render_template("result.html", result=result,result2=result2,data1=dataobt)
